...
import { expenseDocumentsTable, expenseLineItemsTable } from '../database-service/tables/index.js'
...
describe('Expenses service', () => {
	...
	describe('list expense documents by user id', () => {
		it('returns an empty list when the user has no expense documents', async () => {
			...
			await expect(expensesService.listExpenseDocumentsByUserId(user.id)).resolves.toEqual({ data: [], pagination: {} })
		})
	...
	describe('create expense by user id', () => {
		it('creates expense document with computed total and line items', async () => {
			...
			await expect(expensesService.createExpenseByUserId(user.id, payload)).resolves.toEqual({
		...
		it('exposes newly created document in user list response', async () => {
			...
			const result = await expensesService.listExpenseDocumentsByUserId(user.id)
		...
		it('throws when user does not exist', async () => {
			...
			await expect(expensesService.createExpenseByUserId(missingUserId, payload)).rejects.toThrow()
		})
	...
	describe('delete expense by user id', () => {
		it("deletes only user's own expense and returns success response", async () => {
			...
			await expect(expensesService.deleteExpenseByUserId(owner.id, ownerExpense.id)).resolves.toEqual({
		...
		it('throws when expense does not belong to the user', async () => {
			...
			await expect(expensesService.deleteExpenseByUserId(owner.id, otherExpense.id)).rejects.toThrow(
				'Expense not found',
			)
	...
	describe('update expense by user id', () => {
		it('updates expense date, total amount, and replaces line items', async () => {
			...
			await expect(expensesService.updateExpenseByUserId(user.id, expense.id, payload)).resolves.toEqual({
				data: { message: 'expense_updated' },
			})

			const [docAfter] = await db.select().from(expenseDocumentsTable).where(eq(expenseDocumentsTable.id, expense.id))
